...
class Lexer():
    ...
    def _add_tokens(self):
        ######################################################################################################################################################
        # LOGICOMP
        self.lexer.add('DEMORGAN1', r'demorgan1')
        self.lexer.add('DEMORGAN2', r'demorgan2')
        self.lexer.add('DEMORGAN3', r'demorgan3')
        self.lexer.add('DEMORGAN4', r'demorgan4')
        self.lexer.add('CONTRAPOSITIVE', r'contrapositive')
        self.lexer.add('DISJUNCTIVE_SYLLOGISM', r'disj_syllogism')
        ######################################################################################################################################################
        #Comma
...
class constants:
    ...
    # LOGICOMP
    INVALID_DEMORGAN_FORMULA = 44
    INVALID_CONTRAPOSITIVE = 45
    INVALID_DISJUNCTIVE_SYLLOGISM = 46
...
class DeMorgan1Def:
    ...
    def evaluation(self, parser, deduction_result):
        ...
        if not (isinstance(formula1, NegationFormula) and isinstance(formula1.formula, AndFormula)):
            deduction_result.add_error("A fórmula referenciada na linha {} não é da forma ¬(A ∧ B)".format(self.reference1.value))
            return
    ...
    def toLatex(self, symbol_table):
        return '\\infer[\\text{DeMorgan1}]{' + self.formula.toLatex() + '}{' + symbol_table.get_rule(self.reference1.value).toLatex(symbol_table) + '}'
...
class DeMorgan2Def:
    ...
    def evaluation(self, parser, deduction_result):
        ...
        if not (isinstance(formula1, NegationFormula) and isinstance(formula1.formula, OrFormula)):
            deduction_result.add_error("A fórmula referenciada na linha {} não é da forma ¬(A ∨ B)".format(self.reference1.value))
            return
    ...
    def toLatex(self, symbol_table):
        return '\\infer[\\text{DeMorgan2}]{' + self.formula.toLatex() + '}{' + symbol_table.get_rule(self.reference1.value).toLatex(symbol_table) + '}'
...
class DeMorgan3Def:
    ...
    def evaluation(self, parser, deduction_result):
        ...
        if not isinstance(formula1, OrFormula) or not (isinstance(formula1.left, NegationFormula) and isinstance(formula1.right, NegationFormula)):
            deduction_result.add_error("A fórmula referenciada na linha {} não é da forma ¬A ∨ ¬B".format(self.reference1.value))
            return
    ...
    def toLatex(self, symbol_table):
        return '\\infer[\\text{DeMorgan3}]{' + self.formula.toLatex() + '}{' + symbol_table.get_rule(self.reference1.value).toLatex(symbol_table) + '}'
...
class DeMorgan4Def:
    ...
    def evaluation(self, parser, deduction_result):
        ...
        if not isinstance(formula1, AndFormula) or not (isinstance(formula1.left, NegationFormula) and isinstance(formula1.right, NegationFormula)):
            deduction_result.add_error("A fórmula referenciada na linha {} não é da forma ¬A ∧ ¬B".format(self.reference1.value))
            return
...
class ContrapositiveDef:
    ...
    def evaluation(self, parser, deduction_result):
        ...
        if not isinstance(formula1, ImplicationFormula):
            deduction_result.add_error("A fórmula referenciada na linha {} não é uma implicação".format(self.reference1.value))
            return

        A = formula1.left
        B = formula1.right

        expected_formula = ImplicationFormula(NegationFormula(B), NegationFormula(A))
        if self.formula != expected_formula:
            deduction_result.add_error("A fórmula na linha {} não é ¬B → ¬A".format(self.line))
            return

    def toLatex(self, symbol_table):
        return '\\infer[\\text{Contrapositive}]{' + self.formula.toLatex() + '}{' + symbol_table.get_rule(self.reference1.value).toLatex(symbol_table) + '}'
...
class DisjunctiveSyllogismDef:
    ...
    def __init__(self, line, formula, reference1, reference2):
        self.line = line
        self.formula = formula
        self.reference1 = reference1  # A ∨ B
        self.reference2 = reference2  # ¬B ou ¬A
        self.is_copied = False
    ...
    def evaluation(self, parser, deduction_result):
        ...
        formula1 = parser.symbol_table.lookup_formula_by_line(self.line, self.reference1.value)  # A ∨ B
        formula2 = parser.symbol_table.lookup_formula_by_line(self.line, self.reference2.value)  # ¬B ou ¬A
        if formula1 is None or formula2 is None:
            return

        if not isinstance(formula1, OrFormula):
            deduction_result.add_error("A fórmula referenciada na linha {} não é uma disjunção".format(self.reference1.value))
            return

        if not isinstance(formula2, NegationFormula):
            deduction_result.add_error("A fórmula referenciada na linha {} não é uma negação".format(self.reference2.value))
            return

        A = formula1.left
        B = formula1.right
        negated = formula2.formula

        # Variação 1: A ∨ B, ¬B ⊢ A
        if negated == B and self.formula == A:
            return
        # Variação 2: A ∨ B, ¬A ⊢ B
        elif negated == A and self.formula == B:
            return
        else:
            deduction_result.add_error("A fórmula na linha {} não é consistente com o silogismo disjuntivo".format(self.line))
            return

    def toLatex(self, symbol_table):
        return '\\infer[\\text{DisjSyllogism}]{' + self.formula.toLatex() + '}{{' + symbol_table.get_rule(self.reference1.value).toLatex(symbol_table) + '}&{' + symbol_table.get_rule(self.reference2.value).toLatex(symbol_table) + '}}'
######################################################################################################################################################
    ...
    def __init__(self, state):
        ...
        self.pg = ParserGenerator(
            ...
            ['NUM', 'DOT', 'COMMA', 'OPEN_PAREN', 'CLOSE_PAREN', 'NOT', 'RAA',
             ...
             'VAR', 'EXT', 'ALL', 'ALL_ELIM', 'EXT_INTROD', 'EXT_ELIM', 'ALL_INTROD' , 'DEMORGAN1', 'DEMORGAN2', 'DEMORGAN3', 'DEMORGAN4', 'CONTRAPOSITIVE', 'DISJUNCTIVE_SYLLOGISM'],
            # LOGICOMP (o que foi adicionado foi somente o que está depois de 'ALL_INTROD')
            ######################################################################################################################################################
        ...
        def program(p):
            ...
            for i in rule_info:
                ...
                # LOGICOMP
                    ...
                    rule.evaluation(self, deduction_result)
                elif isinstance(rule, DeMorgan3Def):
                    rule.evaluation(self, deduction_result)
                elif isinstance(rule, DeMorgan4Def):
                    rule.evaluation(self, deduction_result)
                elif isinstance(rule, ContrapositiveDef):
                    rule.evaluation(self, deduction_result)
...
#            return p[0], formula_result[0]
        ...
        def DeMorgan1(p):
            formula_result = p[2]
            formula = formula_result[1]
            rule = DeMorgan1Def(p[0].value, formula, p[4])
            self.symbol_table.insert(rule, p[0])
            self.box_latex += "{} & DeMorgan1 {}\\\\\n".format(formula.toLatex(), p[4].value)
            return p[0], formula_result[0]

        @self.pg.production('step : NUM DOT formula DEMORGAN2 NUM')
        def DeMorgan2(p):
            formula_result = p[2]
            formula = formula_result[1]
            rule = DeMorgan2Def(p[0].value, formula, p[4])
            self.symbol_table.insert(rule, p[0])
            self.box_latex += "{} & DeMorgan2 {}\\\\\n".format(formula.toLatex(), p[4].value)
            return p[0], formula_result[0]

        @self.pg.production('step : NUM DOT formula DEMORGAN3 NUM')
        def DeMorgan3(p):
            formula_result = p[2]
            formula = formula_result[1]
            rule = DeMorgan3Def(p[0].value, formula, p[4])
            self.symbol_table.insert(rule, p[0])
            self.box_latex += "{} & DeMorgan3 {}\\\\\n".format(formula.toLatex(), p[4].value)
            return p[0], formula_result[0]

        @self.pg.production('step : NUM DOT formula DEMORGAN4 NUM')
        def DeMorgan4(p):
            formula_result = p[2]
            formula = formula_result[1]
            rule = DeMorgan4Def(p[0].value, formula, p[4])
            self.symbol_table.insert(rule, p[0])
            self.box_latex += "{} & DeMorgan4 {}\\\\\n".format(formula.toLatex(), p[4].value)
            return p[0], formula_result[0]

        @self.pg.production('step : NUM DOT formula CONTRAPOSITIVE NUM')
        def Contrapositive(p):
            formula_result = p[2]
            formula = formula_result[1]
            rule = ContrapositiveDef(p[0].value, formula, p[4])
            self.symbol_table.insert(rule, p[0])
            self.box_latex += "{} & Contrapositive {}\\\\\n".format(formula.toLatex(), p[4].value)
            return p[0], formula_result[0]

        @self.pg.production('step : NUM DOT formula DISJUNCTIVE_SYLLOGISM NUM COMMA NUM')
        def DisjunctiveSyllogism(p):
            formula_result = p[2]
            formula = formula_result[1]
            rule = DisjunctiveSyllogismDef(p[0].value, formula, p[4], p[6])
            self.symbol_table.insert(rule, p[0])
            self.box_latex += "{} & DisjSyllogism {}, {}\\\\\n".format(formula.toLatex(), p[4].value, p[6].value)
            return p[0], formula_result[0]
        ######################################################################################################################################################
